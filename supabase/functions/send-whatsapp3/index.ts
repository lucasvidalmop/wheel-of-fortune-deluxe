...
serve(async (req) => {
  ...
  try {
    ...
    if (!evolutionApiUrl || !evolutionApiKey || !evolutionInstance) {
      return new Response(
        JSON.stringify({ error: "Credenciais da Evolution API (instância 3) não configuradas." }),
        { status: 200, headers: { ...corsHeaders, "Content-Type": "application/json" } }
      );
    }
    ...
    try {
      ...
      return new Response(
        JSON.stringify({
          error: isTimeout
            ? "Tempo limite excedido ao enviar mensagem. Verifique a Evolution API (instância 3)."
    ...
    if (!response.ok) {
      console.error("Evolution API (instance 3) error:", data);
      return new Response(
        JSON.stringify({ error: data?.message || data?.error || "Erro ao enviar WhatsApp (instância 3)" }),
        { status: 200, headers: { ...corsHeaders, "Content-Type": "application/json" } }
      );
    }
    ...
    console.error("WhatsApp 3 error:", error);
    return new Response(
