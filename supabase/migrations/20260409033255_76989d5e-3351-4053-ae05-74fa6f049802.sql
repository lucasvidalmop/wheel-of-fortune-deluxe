...
-- Add new column for multiple prize segments
ALTER TABLE public.referral_links ADD COLUMN IF NOT EXISTS fixed_prize_segments jsonb DEFAULT NULL;

-- Migrate existing single segment data to new array format
UPDATE public.referral_links
SET fixed_prize_segments = jsonb_build_array(fixed_prize_segment)
WHERE fixed_prize_segment IS NOT NULL AND fixed_prize_segments IS NULL;

-- Update register_via_referral function (the one with cpf param) to use new array
...
BEGIN
  ...
  -- Determine fixed prize: use new array field, fallback to legacy single field
