-- Corrige condicao de corrida em auto_credit_luckybox_grant: a versao
-- anterior fazia SELECT (sem lock) e so depois UPDATE, entao duas chamadas
-- quase simultaneas (duplo-clique, retry, webhook repetido) podiam passar
-- pela checagem "status = pending" ao mesmo tempo e creditar a caixa 2x.
-- Agora a reivindicacao da linha (status pending -> redeemed) e atomica:
-- so uma chamada consegue "ganhar" o UPDATE condicional, a outra recebe
-- 'Already processed' sem creditar nada.
...
BEGIN
  ...
  -- Reivindicacao atomica: so segue quem conseguir virar pending -> redeemed
  -- nesta unica instrucao. Qualquer chamada concorrente/repetida encontra
  -- 0 linhas aqui e sai sem creditar de novo.
