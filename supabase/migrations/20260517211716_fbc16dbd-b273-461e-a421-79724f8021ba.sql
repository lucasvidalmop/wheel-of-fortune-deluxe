-- Remove ambas as versões antigas para evitar overload
DROP FUNCTION IF EXISTS public.update_wheel_user_self(uuid, text, text, text, text, text, text, text, jsonb, text);
DROP FUNCTION IF EXISTS public.update_wheel_user_self(uuid, text, text, text, text, text, text, text, jsonb, text, text);
...
BEGIN
  ...
  -- Verifica se existe um registro em referral_redemptions com mesmo email + cpf + owner
  SELECT EXISTS (
  ...
  -- Busca o wheel_user correspondente (por email + owner)
  ...
  v_new_account := btrim(coalesce(p_new_account_id, ''));
  IF v_allow_account AND v_new_account <> '' AND v_new_account <> v_user.account_id THEN
    SELECT id INTO v_conflict_id FROM public.wheel_users
     WHERE owner_id = p_owner_id AND account_id = v_new_account AND id <> v_user.id
     LIMIT 1;
  ...
  UPDATE public.wheel_users SET
    ...
    account_id = CASE WHEN v_allow_account AND v_new_account <> '' THEN v_new_account ELSE account_id END,
    updated_at = now()
  WHERE id = v_user.id;
