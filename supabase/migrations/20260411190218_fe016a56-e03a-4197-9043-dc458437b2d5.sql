...
-- Add spins expiration column
ALTER TABLE public.wheel_users ADD COLUMN IF NOT EXISTS spins_expire_at timestamp with time zone DEFAULT NULL;

-- Update get_wheel_user_spins to auto-expire spins
...
-- Update authenticate_wheel_user to auto-expire spins
...
-- Update register_via_referral (the version with phone/pix params) to set expiration
...
BEGIN
  ...
  -- Load spin expiration config from wheel_configs
