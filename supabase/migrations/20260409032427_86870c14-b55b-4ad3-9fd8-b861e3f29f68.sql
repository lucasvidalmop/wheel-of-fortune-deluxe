...
-- Add columns to referral_links
ALTER TABLE public.referral_links
  ADD COLUMN IF NOT EXISTS fixed_prize_segment integer DEFAULT NULL,
  ADD COLUMN IF NOT EXISTS auto_payment boolean NOT NULL DEFAULT false;

-- Update the register_via_referral function (the one with cpf param) to set fixed_prize and auto_payment
...
BEGIN
  ...
  -- Update spins and apply fixed prize + auto_payment from the referral link
  UPDATE public.wheel_users
  SET spins_available = spins_available + v_link.spins_per_registration,
      fixed_prize_enabled = CASE WHEN v_link.fixed_prize_segment IS NOT NULL THEN true ELSE fixed_prize_enabled END,
      fixed_prize_segment = CASE WHEN v_link.fixed_prize_segment IS NOT NULL THEN v_link.fixed_prize_segment ELSE wheel_users.fixed_prize_segment END,
