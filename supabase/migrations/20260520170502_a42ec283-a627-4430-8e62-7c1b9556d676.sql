...
BEGIN
  ...
  -- operator limits from bets_configs.page_config->'multiTicket'
  ...
  IF p_amount < v_op_min_bet THEN
    RETURN jsonb_build_object('success', false, 'error', 'stake_below_min');
  END IF;
  IF v_op_max_bet > 0 AND p_amount > v_op_max_bet THEN
    RETURN jsonb_build_object('success', false, 'error', 'stake_above_max');
  END IF;
  ...
  FOR v_sel IN SELECT * FROM jsonb_array_elements(p_selections) LOOP
    ...
    IF v_outcome.event_id::text = ANY(v_events) THEN
      v_same_fixture := true;
    END IF;
  ...
  -- house factor (mais restritivo dentre os aplicáveis)
  ...
  IF v_final > v_max_odd THEN
    RETURN jsonb_build_object('success', false, 'error', 'odd_above_max', 'max_odd', v_max_odd);
  END IF;

  v_potential := ROUND(p_amount * v_final)::int;

  IF v_op_max_return > 0 AND v_potential > v_op_max_return THEN
    RETURN jsonb_build_object('success', false, 'error', 'return_above_max', 'max_return', v_op_max_return);
  END IF;
