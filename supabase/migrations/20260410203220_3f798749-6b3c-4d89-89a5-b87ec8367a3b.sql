...
BEGIN
  ...
  -- First try exact match (email + account_id + owner)
  ...
  -- If not found, try by account_id + owner (covers different email same account)
  ...
  -- If still not found, try by email + owner (covers same email different account)
  IF v_user.id IS NULL THEN
    SELECT * INTO v_user FROM public.wheel_users
    WHERE lower(btrim(email)) = lower(btrim(p_email))
      AND owner_id = v_link.owner_id;
  ...
  -- Only create new user if truly not found
  IF v_user.id IS NULL THEN
