...
-- Classificador de mercado a partir do título (espelha src/lib/ticketCoherence.ts)
CREATE OR REPLACE FUNCTION public._classify_bet_market(p_title text)
RETURNS text
LANGUAGE plpgsql IMMUTABLE SET search_path = public AS $$
DECLARE t text;
BEGIN
  t := lower(coalesce(p_title, ''));
  IF t = '' OR t = 'principal' THEN RETURN 'final'; END IF;
  IF t ~ 'dupla\s*chance|double\s*chance' THEN RETURN 'dc'; END IF;
  IF t ~ 'draw\s*no\s*bet|empate\s*anula|sem\s*empate|dnb' THEN RETURN 'dnb'; END IF;
  IF t ~ 'ambos.*marcam|both.*score|btts' THEN RETURN 'btts'; END IF;
  IF t ~ 'placar.*correto|correct.*score' THEN RETURN 'cs'; END IF;
  IF t ~ 'handicap|asi[áa]tico' THEN RETURN 'ah'; END IF;
  IF t ~ 'mais\s*de|menos\s*de|over|under|total.*gols|mais\/menos|over\/under' THEN RETURN 'ou'; END IF;
  IF t ~ 'resultado.*final|1x2|moneyline|vencedor|match.*winner|full.*time' THEN RETURN 'final'; END IF;
  RETURN 'other';
END;
$$;

CREATE OR REPLACE FUNCTION public._pair_blocked(a text, b text)
RETURNS boolean
LANGUAGE sql IMMUTABLE SET search_path = public AS $$
  SELECT EXISTS (
    SELECT 1 FROM (VALUES
      ('final','dc'),('final','dnb'),('final','ah'),('final','cs'),
      ('dc','dnb'),('dc','cs'),('dc','ah'),
      ('btts','cs'),('ou','cs')
    ) AS p(x,y)
    WHERE (p.x = a AND p.y = b) OR (p.x = b AND p.y = a)
  );
$$;
...
DECLARE
  ...
  v_max_odd numeric;
  -- coherence tracking
  v_fix_kinds jsonb := '{}'::jsonb;  -- event_id -> array of kinds
  v_existing jsonb;
  v_kind text;
  v_other text;
  v_idx integer;
BEGIN
  ...
  v_allow_same_fixture := COALESCE((v_cfg->'multiTicket'->>'allowSameFixture')::boolean, false);
  ...
  FOR v_sel IN SELECT * FROM jsonb_array_elements(p_selections) LOOP
    ...
    IF v_outcome.market_id IS NOT NULL THEN
      ...
      v_market_title := COALESCE(v_market.title, '');
    ...
    -- coherence check per fixture
    v_kind := public._classify_bet_market(v_market_title);
    v_existing := v_fix_kinds->(v_outcome.event_id::text);
    IF v_existing IS NOT NULL THEN
      IF NOT v_allow_same_fixture THEN
        RETURN jsonb_build_object('success', false, 'error', 'same_fixture_not_allowed');
      END IF;
      FOR v_idx IN 0 .. jsonb_array_length(v_existing) - 1 LOOP
        v_other := v_existing->>v_idx;
        IF public._pair_blocked(v_kind, v_other) THEN
          RETURN jsonb_build_object('success', false, 'error', 'incoherent_selections');
        END IF;
      END LOOP;
      v_fix_kinds := jsonb_set(v_fix_kinds, ARRAY[v_outcome.event_id::text], v_existing || to_jsonb(v_kind));
    ELSE
      v_fix_kinds := jsonb_set(v_fix_kinds, ARRAY[v_outcome.event_id::text], jsonb_build_array(v_kind), true);
    END IF;
