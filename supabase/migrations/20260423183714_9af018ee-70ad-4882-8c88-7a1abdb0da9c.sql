...
ADD COLUMN IF NOT EXISTS fixed_prize_plan jsonb;

ALTER TABLE public.wheel_users
ADD COLUMN IF NOT EXISTS fixed_prize_queue jsonb NOT NULL DEFAULT '[]'::jsonb;

UPDATE public.wheel_users
SET fixed_prize_queue = CASE
  WHEN fixed_prize_enabled = true AND fixed_prize_segment IS NOT NULL THEN jsonb_build_array(fixed_prize_segment)
  ELSE '[]'::jsonb
END
WHERE COALESCE(fixed_prize_queue, '[]'::jsonb) = '[]'::jsonb;

CREATE OR REPLACE FUNCTION public.build_fixed_prize_queue(
  p_plan jsonb,
  p_spins integer,
  p_legacy_segments jsonb DEFAULT NULL,
  p_legacy_segment integer DEFAULT NULL
)
RETURNS jsonb
LANGUAGE plpgsql
VOLATILE
...
BEGIN
  ...
  SET spins_available = 0, fixed_prize_enabled = false, fixed_prize_segment = NULL, fixed_prize_queue = '[]'::jsonb, spins_expire_at = NULL, updated_at = now()
