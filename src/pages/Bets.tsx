...
const Bets = ({ tag }: BetsPageProps) => {
  ...
  const pageBackgroundLayerStyle: React.CSSProperties = cfg.bgImage
    ? { backgroundImage: `url(${optimizedImage(cfg.bgImage, { width: 1920, quality: 90 })})`, backgroundSize: 'cover', backgroundPosition: 'center', backgroundRepeat: 'no-repeat' }
    : (cfg.bgGradientFrom || cfg.bgGradientTo)
      ? { background: `radial-gradient(ellipse at top, ${cfg.bgGradientFrom || '#1a1230'} 0%, ${cfg.bgGradientTo || '#05040a'} 70%)` }
      : { background: bg };

  return (
    <div className={`relative isolate ${lobbyEmbed ? '' : 'min-h-screen'} overflow-x-hidden`} style={lobbyEmbed ? { color: text } : { background: bg, color: text }}>
                            ...
                            return (
                              <div key={o.id} className="relative">
                                <button
                                  ...
                                  <div className="text-[9px] uppercase tracking-[0.15em] font-bold mb-0.5 truncate pr-6" style={{ color: muted }}>{(() => {
                                    const lbl = outcomeLabelForCard(o.label, ev.title);
                                    const parts = lbl.match(/^(\S+)\s+([+\-]?\d[\d.,/]*|\d+:\d+|\([^)]+\))$/);
                                    if (parts) {
                                      return (<><span>{parts[1]}</span><span className="ml-1 opacity-60 font-semibold">{parts[2]}</span></>);
                                    }
                                    return lbl;
                                  })()}</div>
          ...
          >
            ...
            {ticketDraft.length === 0 ? (
              ...
              <>
                ...
                <input
                  type="number" onFocus={(e) => e.target.select()} min={minBetAllowed} max={maxBetAllowed} value={ticketAmount}
