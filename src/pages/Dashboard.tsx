...
const WhatsappActivityPanel = lazy(() => import('@/components/casino/WhatsappActivityPanel'));
...
interface PersistedDashboardSettings {
  ...
  evolutionApiUrl3: string;
  evolutionApiKey3: string;
  evolutionInstance3: string;
...
function Dashboard() {
  ...
  const [toolPerms, setToolPerms] = useState<Record<string, boolean>>({
    roleta: true, sms: true, sms_mb: true, sms_cs: true, email: true, whatsapp: true, whatsapp2: true, whatsapp3: true, financeiro: true, gorjeta: true, referral: true,
  ...
  const [activeTab, setActiveTab] = useState<'inscritos' | 'wheel' | 'batalha_slot' | 'luckybox' | 'apostas' | 'lobby' | 'sorteio' | 'sorteio_whatsapp' | 'auth' | 'history' | 'email' | 'email_brevo' | 'sms' | 'sms_cs' | 'whatsapp' | 'whatsapp2' | 'whatsapp3' | 'analytics' | 'financeiro' | 'referral' | 'notificacoes' | 'gorjeta' | 'hist_gorjeta' | 'configuracoes' | 'painel_casa' | 'deposito' | 'hist_deposito' | 'msg_analytics'>('inscritos');
  ...
  const [form, setForm] = useState({ account_id: '', email: '', name: '', phone: '', cpf: '', fixed_prize_enabled: false, fixed_prize_segment: null as number | null, pix_key_type: '', pix_key: '', user_type: '', responsible: '', auto_payment: false });
  ...
  // WhatsApp 3 state (terceira instância — credenciais e envios independentes)
  const [whatsappSending3, setWhatsappSending3] = useState(false);
  const [showWhatsappConfig3, setShowWhatsappConfig3] = useState(false);
  const [evolutionApiUrl3, setEvolutionApiUrl3] = useState('');
  const [evolutionApiKey3, setEvolutionApiKey3] = useState('');
  const [evolutionInstance3, setEvolutionInstance3] = useState('');
  const [instanceStatus3, setInstanceStatus3] = useState<'unknown' | 'loading' | 'open' | 'close' | 'connecting' | 'error'>('unknown');
  const [instanceQrCode3, setInstanceQrCode3] = useState<string | null>(null);
  const [creatingInstance3, setCreatingInstance3] = useState(false);
  const [whatsappLogs3, setWhatsappLogs3] = useState<any[]>([]);
  const [whatsappLogsLoading3, setWhatsappLogsLoading3] = useState(false);
  const [showWhatsappHistory3, setShowWhatsappHistory3] = useState(false);
  const [notifyGroups3, setNotifyGroups3] = useState<{ id: string; subject: string }[]>([]);
  const [notifySelectedGroups3, setNotifySelectedGroups3] = useState<{ id: string; subject: string }[]>([]);
  const [notifyGroupJid3, setNotifyGroupJid3] = useState('');
  const [notifyGroupName3, setNotifyGroupName3] = useState('');
  const [notifyGroupsLoading3, setNotifyGroupsLoading3] = useState(false);
  ...
  const [whatsappProgress3, setWhatsappProgress3] = useState<BulkProgress>(emptyProgress);
  ...
  const whatsapp3Ctrl = useBulkSendControl();
  const whatsapp3GroupCtrl = useBulkSendControl();
  ...
  const fetchScheduledMessages = async () => {
    ...
    const [pendingRes, recentRes] = await Promise.all([
      supabase.from('scheduled_messages').select('*').eq('owner_id', session.user.id).in('channel', ['whatsapp', 'whatsapp2', 'whatsapp3'] as any).eq('status', 'pending').order('next_run_at', { ascending: true }).limit(500),
      supabase.from('scheduled_messages').select('*').eq('owner_id', session.user.id).in('channel', ['whatsapp', 'whatsapp2', 'whatsapp3'] as any).neq('status', 'pending').order('scheduled_at', { ascending: false }).limit(200),
    ]);
  ...
  const saveScheduledMessage = async () => {
    ...
    const scheduleChannel = activeTab === 'whatsapp3' ? 'whatsapp3' : activeTab === 'whatsapp2' ? 'whatsapp2' : 'whatsapp';
  ...
  const cancelSmsScheduleBatchByGroup = async (batch: ScheduledBatch<any>) => {
    if (!session?.user?.id) return;
    const m = batch.sample;
    const { error } = await supabase.from('scheduled_messages')
      .update({ status: 'cancelled', updated_at: new Date().toISOString() } as any)
      .eq('owner_id', session.user.id)
      .eq('channel', m.channel as any)
      .eq('status', 'pending')
  ...
  useEffect(() => {
    if (!session?.user?.id) return;
    if (activeTab === 'whatsapp' || activeTab === 'whatsapp2' || activeTab === 'whatsapp3') {
      fetchScheduledMessages();
    }
    if (activeTab === 'sms') {
  ...
  const applyPersistedDashboardSettings = (rawSettings?: Partial<PersistedDashboardSettings>) => {
    ...
    setEvolutionApiUrl3(settings.evolutionApiUrl3 || '');
    setEvolutionApiKey3(settings.evolutionApiKey3 || '');
    setEvolutionInstance3(settings.evolutionInstance3 || '');
  ...
  const fetchWhatsappLogs3 = async () => {
    if (!session?.user?.id) return;
    setWhatsappLogsLoading3(true);
    const { data } = await (supabase as any)
      .from('whatsapp3_message_log')
      .select('*')
      .eq('owner_id', session.user.id)
      .order('created_at', { ascending: false })
      .limit(100);
    setWhatsappLogs3(data || []);
    setWhatsappLogsLoading3(false);
  };
  ...
  const allMenuItems: { key: typeof activeTab; icon: React.ReactNode; label: string; tool?: 'roleta' | 'sms' | 'sms_cs' | 'email' | 'email_brevo' | 'whatsapp' | 'whatsapp2' | 'whatsapp3' | 'financeiro' | 'gorjeta' | 'referral' | 'inscritos' | 'auth' | 'history' | 'analytics' | 'msg_analytics' | 'notificacoes' | 'configuracoes' | 'painel_casa' | 'batalha_slot' | 'luckybox' | 'apostas' | 'sorteio' }[] = [
    ...
    { key: 'sorteio', icon: <Gift size={20} />, label: 'Sorteio ao Vivo', tool: 'sorteio' },
    { key: 'sorteio_whatsapp', icon: <MessageCircle size={20} />, label: 'Sorteio WhatsApp', tool: 'sorteio' },
    ...
    { key: 'whatsapp3', icon: <MessageCircle size={20} />, label: 'WhatsApp 3', tool: 'whatsapp3' },
  ...
  const groupDefs: { key: GroupKey; label: string; itemKeys: typeof activeTab[] }[] = [
    { key: 'operacao', label: 'Operação', itemKeys: ['inscritos', 'wheel', 'batalha_slot', 'luckybox', 'apostas', 'lobby', 'sorteio', 'sorteio_whatsapp', 'auth', 'history'] },
    { key: 'disparos', label: 'Disparos', itemKeys: ['email', 'email_brevo', 'sms', 'sms_cs', 'whatsapp', 'whatsapp2', 'whatsapp3', 'msg_analytics'] },
  ...
  const tabTitles: Record<string, string> = {
    ...
    sorteio_whatsapp: 'Sorteio WhatsApp',
    apostas: 'Apostas',
    ...
    whatsapp3: 'Disparo de WhatsApp 3',
  ...
  return (
    <div className="min-h-screen bg-background flex relative overflow-x-hidden overflow-y-auto">
      ...
      {/* ═══ MAIN CONTENT ═══ */}
      <div className={`flex-1 min-w-0 pt-14 lg:pt-0 p-3 md:p-4 transition-all duration-500 relative z-10 ${sidebarCollapsed ? 'lg:ml-[72px]' : 'lg:ml-[260px]'}`}>
        <div className="w-full max-w-[1600px] min-w-0 mx-auto lg:py-4 space-y-4">
                        ...
                        <div key={batch.key} className={`p-3 rounded-xl border text-xs space-y-1 ${m.status === 'pending' ? 'border-primary/20 bg-primary/5' : m.status === 'sent' ? 'border-green-500/20 bg-green-500/5' : m.status === 'cancelled' ? 'border-muted/20 bg-muted/5 opacity-60' : 'border-red-500/20 bg-red-500/5'}`}>
                          ...
                          <div className="flex justify-between items-center pt-1">
                            ...
                            <div className="flex items-center gap-2 ml-2 flex-shrink-0">
                              ...
                              {m.status === 'pending' && <button onClick={() => cancelSmsScheduleBatchByGroup(batch)} className="text-red-400 hover:text-red-300 transition text-[10px] font-bold">Cancelar</button>}
                            </div>
                          </div>
                        </div>
                      )})}
          ...
          {activeTab === 'whatsapp' && (
            <div className="max-w-2xl space-y-5">
              ...
              <GlassCard className="p-5 space-y-3">
                ...
                {(notifySelectedGroups.length > 0 || notifySelectedGroups2.length > 0 || notifySelectedGroups3.length > 0) && (
          ...
          {activeTab === 'whatsapp3' && (
            <div className="max-w-2xl space-y-5">
              <div className="flex items-center gap-2 justify-end">
                <button onClick={() => { setShowWhatsappHistory3(!showWhatsappHistory3); if (!showWhatsappHistory3) fetchWhatsappLogs3(); }} className={`flex items-center gap-1.5 px-4 py-2 rounded-xl border text-sm transition ${showWhatsappHistory3 ? 'border-primary/30 bg-primary/10 text-primary' : 'border-white/[0.08] bg-white/[0.04] text-muted-foreground hover:text-foreground hover:bg-white/[0.08]'}`}>
                  <Clock size={15} /> Histórico
                </button>
                <button onClick={() => setShowWhatsappConfig3(!showWhatsappConfig3)} className={`flex items-center gap-1.5 px-4 py-2 rounded-xl border text-sm transition ${showWhatsappConfig3 ? 'border-primary/30 bg-primary/10 text-primary' : 'border-white/[0.08] bg-white/[0.04] text-muted-foreground hover:text-foreground hover:bg-white/[0.08]'}`}>
                  <Settings size={15} /> Configurar API
                </button>
              </div>
              ...
              {showWhatsappConfig3 && (
                <GlassCard className="p-5 space-y-4">
                  ...
                  <div className="space-y-1">
                    <label className="text-xs text-muted-foreground">URL da API</label>
                    <input type="text" value={evolutionApiUrl3} onChange={e => setEvolutionApiUrl3(e.target.value)} placeholder="https://sua-api.com" className="w-full px-3 py-2 rounded-xl border border-white/[0.08] bg-white/[0.04] text-foreground text-sm font-mono focus:outline-none focus:ring-1 focus:ring-primary/40" />
                  </div>
                  <div className="space-y-1">
                    <label className="text-xs text-muted-foreground">API Key</label>
                    <input type="password" value={evolutionApiKey3} onChange={e => setEvolutionApiKey3(e.target.value)} placeholder="••••••••" className="w-full px-3 py-2 rounded-xl border border-white/[0.08] bg-white/[0.04] text-foreground text-sm font-mono focus:outline-none focus:ring-1 focus:ring-primary/40" />
                  </div>
                  <div className="space-y-1">
                    <label className="text-xs text-muted-foreground">Nome da Instância</label>
                    <input type="text" value={evolutionInstance3} onChange={e => setEvolutionInstance3(e.target.value)} placeholder="minha-instancia" className="w-full px-3 py-2 rounded-xl border border-white/[0.08] bg-white/[0.04] text-foreground text-sm font-mono focus:outline-none focus:ring-1 focus:ring-primary/40" />
                  </div>
                  ...
                  <div className="border-t border-white/[0.06] pt-4 space-y-3">
                    ...
                    <div className="flex gap-2 flex-wrap">
                      {/* Criar Instância */}
                      <button
                        disabled={!evolutionApiUrl3 || !evolutionApiKey3 || !evolutionInstance3 || creatingInstance3}
                        onClick={async () => {
                          setCreatingInstance3(true);
                          try {
                            const { data, error } = await supabase.functions.invoke('evolution-proxy', {
                              body: { action: 'create', evolutionApiUrl: evolutionApiUrl3, evolutionApiKey: evolutionApiKey3, evolutionInstance: evolutionInstance3 }
                            });
                            const msg = data?.error || data?.data?.error || data?.data?.response?.message || data?.data?.message || error?.message || 'Erro ao criar instância';
                            if (error || !data?.ok) {
                              toast.error(Array.isArray(msg) ? msg.join(', ') : msg);
                              setInstanceStatus3('error');
                              return;
                            }
                            const d = data.data;
                            toast.success('Instância criada com sucesso!');
                            if (d?.qrcode?.base64) {
                              setInstanceQrCode3(d.qrcode.base64);
                              setInstanceStatus3('connecting');
                            } else {
                              setInstanceStatus3('close');
                            }
                          } catch (err: any) {
                            toast.error(err.message || 'Erro de conexão');
                            setInstanceStatus3('error');
                          } finally {
                            setCreatingInstance3(false);
                          }
                        }}
                        className="flex-1 min-w-[120px] px-3 py-2.5 rounded-xl text-xs font-semibold border border-white/[0.08] bg-white/[0.04] text-foreground hover:bg-white/[0.08] disabled:opacity-40 transition flex items-center justify-center gap-1.5"
                      >
                        <Plus size={14} /> {creatingInstance3 ? 'Criando...' : 'Criar Instância'}
                      </button>

                      {/* Conectar / QR Code */}
                      <button
                        disabled={!evolutionApiUrl3 || !evolutionApiKey3 || !evolutionInstance3 || instanceStatus3 === 'loading'}
                        onClick={async () => {
                          setInstanceStatus3('loading');
                          setInstanceQrCode3(null);
                          try {
                            const { data, error } = await supabase.functions.invoke('evolution-proxy', {
                              body: { action: 'connect', evolutionApiUrl: evolutionApiUrl3, evolutionApiKey: evolutionApiKey3, evolutionInstance: evolutionInstance3 }
                            });
                            const msg = data?.error || data?.data?.error || data?.data?.response?.message || data?.data?.message || error?.message || 'Erro ao conectar';
                            if (error || !data?.ok) {
                              toast.error(Array.isArray(msg) ? msg.join(', ') : msg);
                            ...
                            const d = data.data;
                            if (d?.base64) {
                              setInstanceQrCode3(d.base64);
                              setInstanceStatus3('connecting');
                              toast.info('Escaneie o QR Code no WhatsApp');
                            } else if (d?.instance?.state === 'open') {
                              setInstanceStatus3('open');
                              toast.success('WhatsApp já está conectado!');
                            } else {
                              setInstanceStatus3('close');
                              toast.info('Instância desconectada. Tente novamente.');
                        ...
                        className="flex-1 min-w-[120px] px-3 py-2.5 rounded-xl text-xs font-semibold border border-green-500/30 bg-green-500/10 text-green-400 hover:bg-green-500/20 disabled:opacity-40 transition flex items-center justify-center gap-1.5"
                      >
                        <Smartphone size={14} /> {instanceStatus3 === 'loading' ? 'Conectando...' : 'Conectar (QR Code)'}
                      </button>

                      {/* Verificar Status */}
                      <button
                        disabled={!evolutionApiUrl3 || !evolutionApiKey3 || !evolutionInstance3}
                        onClick={async () => {
                          setInstanceStatus3('loading');
                          try {
                            const { data, error } = await supabase.functions.invoke('evolution-proxy', {
                              body: { action: 'status', evolutionApiUrl: evolutionApiUrl3, evolutionApiKey: evolutionApiKey3, evolutionInstance: evolutionInstance3 }
                            });
                            const msg = data?.error || data?.data?.error || data?.data?.response?.message || data?.data?.message || error?.message || 'Erro ao verificar';
                            if (error || !data?.ok) {
                              toast.error(Array.isArray(msg) ? msg.join(', ') : msg);
                      ...
                      <button
                        ...
                        onClick={async () => {
                          try {
                            const { data, error } = await supabase.functions.invoke('evolution-proxy', {
                              body: { action: 'logout', evolutionApiUrl: evolutionApiUrl3, evolutionApiKey: evolutionApiKey3, evolutionInstance: evolutionInstance3 }
                            });
                            const msg = data?.error || data?.data?.error || data?.data?.response?.message || data?.data?.message || error?.message || 'Erro ao desconectar';
                            if (error || !data?.ok) {
                              toast.error(Array.isArray(msg) ? msg.join(', ') : msg);
                    ...
                    {/* Status badge */}
                    <div className={`text-xs font-medium flex items-center gap-1.5 ${
                      instanceStatus3 === 'open' ? 'text-green-400' :
                      instanceStatus3 === 'connecting' ? 'text-yellow-400' :
                      instanceStatus3 === 'error' ? 'text-red-400' :
                      instanceStatus3 === 'close' ? 'text-orange-400' :
                      'text-muted-foreground'
                    }`}>
                      {instanceStatus3 === 'open' && '🟢 WhatsApp conectado'}
                      {instanceStatus3 === 'connecting' && '🟡 Aguardando leitura do QR Code...'}
                      {instanceStatus3 === 'close' && '🔴 Desconectado'}
                      {instanceStatus3 === 'error' && '❌ Erro na conexão'}
                      {instanceStatus3 === 'loading' && '⏳ Verificando...'}
                      {instanceStatus3 === 'unknown' && (evolutionApiUrl3 && evolutionApiKey3 && evolutionInstance3 ? '⚪ Clique em "Verificar Status"' : '⚠️ Preencha todas as credenciais')}
                    </div>
                    ...
                    {instanceQrCode3 && (
                      <div className="flex flex-col items-center gap-3 p-4 rounded-xl border border-white/[0.08] bg-white/[0.02]">
                        <p className="text-xs text-muted-foreground font-medium">Escaneie o QR Code com o WhatsApp</p>
                        <div className="bg-white p-3 rounded-xl">
                          <img src={instanceQrCode3.startsWith('data:') ? instanceQrCode3 : `data:image/png;base64,${instanceQrCode3}`} alt="QR Code WhatsApp" className="w-56 h-56" />
                        </div>
                        <p className="text-[10px] text-muted-foreground">Abra o WhatsApp → Menu (⋮) → Aparelhos conectados → Conectar aparelho</p>
                        <button onClick={() => { setInstanceQrCode3(null); setInstanceStatus3('unknown'); }} className="text-xs text-muted-foreground hover:text-foreground transition">
                          Fechar QR Code
                        </button>
                      </div>
                    )}
              ...
              <GlassCard className="p-5 space-y-4">
                <div className="flex items-center justify-between">
                  ...
                  <button
                    type="button"
                    disabled={!evolutionApiUrl3 || !evolutionApiKey3 || !evolutionInstance3 || notifyGroupsLoading3}
                    onClick={async () => {
                      setNotifyGroupsLoading3(true);
                      try {
                        const { data } = await supabase.functions.invoke('evolution-proxy', {
                          body: {
                            action: 'fetchGroups',
                            evolutionApiUrl: evolutionApiUrl3, evolutionApiKey: evolutionApiKey3, evolutionInstance: evolutionInstance3,
                          },
                        });
                        if (data?.ok && Array.isArray(data.data)) {
                          setNotifyGroups3(data.data.map((g: any) => ({ id: g.id, subject: g.subject || g.id })));
                          toast.success(`${data.data.length} grupo(s) encontrado(s)`);
                        } else {
                          toast.error('Erro ao buscar grupos. Verifique se a API está conectada.');
                        }
                      ...
                      setNotifyGroupsLoading3(false);
                    }}
                    className="px-4 py-2 rounded-xl text-xs font-semibold border border-green-500/30 bg-green-500/10 text-green-400 hover:bg-green-500/20 disabled:opacity-40 transition flex items-center gap-1.5"
                  >
                    <RotateCcw size={14} /> {notifyGroupsLoading3 ? 'Buscando...' : 'Buscar Grupos'}
                  </button>
                </div>

                {notifyGroups3.length > 0 ? (
                  <div className="space-y-2">
                    <div className="max-h-[200px] overflow-y-auto space-y-1.5 pr-1 [&::-webkit-scrollbar]:w-1.5 [&::-webkit-scrollbar-track]:bg-transparent [&::-webkit-scrollbar-thumb]:bg-white/[0.1] [&::-webkit-scrollbar-thumb]:rounded-full">
                      {notifyGroups3.map(g => {
                        const isSelected = notifySelectedGroups3.some(sg => sg.id === g.id);
                        return (
                          <label key={g.id} className={`flex items-center gap-3 p-2.5 rounded-xl border cursor-pointer transition-all ${isSelected ? 'bg-green-500/10 border-green-500/20' : 'bg-white/[0.02] border-white/[0.06] hover:bg-white/[0.04]'}`}>
                            <input
                              type="checkbox"
                              ...
                              onChange={() => {
                                if (isSelected) {
                                  const updated = notifySelectedGroups3.filter(sg => sg.id !== g.id);
                                  setNotifySelectedGroups3(updated);
                                  if (notifyGroupJid3 === g.id) {
                                    setNotifyGroupJid3(updated[0]?.id || '');
                                    setNotifyGroupName3(updated[0]?.subject || '');
                                  }
                                } else {
                                  const updated = [...notifySelectedGroups3, g];
                                  setNotifySelectedGroups3(updated);
                                  if (!notifyGroupJid3) {
                                    setNotifyGroupJid3(g.id);
                                    setNotifyGroupName3(g.subject);
                                  }
                                }
                              }}
                              className="rounded border-white/20 accent-green-500"
                    ...
                    {notifySelectedGroups3.length > 0 && (
                      <div className="flex items-center justify-between p-2.5 rounded-xl bg-green-500/10 border border-green-500/20">
                        <span className="text-xs font-medium text-green-400">📌 {notifySelectedGroups3.length} grupo(s) selecionado(s)</span>
                        <button type="button" onClick={() => { setNotifySelectedGroups3([]); setNotifyGroupJid3(''); setNotifyGroupName3(''); }} className="text-xs text-red-400 hover:text-red-300">Limpar todos</button>
                      </div>
                    )}
                  </div>
                ) : notifySelectedGroups3.length > 0 ? (
                  <div className="space-y-1.5">
                    {notifySelectedGroups3.map(g => (
                      <div key={g.id} className="flex items-center justify-between p-2.5 rounded-xl bg-white/[0.06] border border-white/[0.08]">
                        <span className="text-xs text-foreground">📌 {g.subject}</span>
                        <button type="button" onClick={() => {
                          const updated = notifySelectedGroups3.filter(sg => sg.id !== g.id);
              ...
              {showWhatsappHistory3 && (
                <GlassCard className="p-5 space-y-4">
                  <div className="flex items-center justify-between">
                    <h3 className="text-sm font-bold text-foreground flex items-center gap-2"><Clock size={16} className="text-primary" /> Histórico de Mensagens</h3>
                    <button onClick={fetchWhatsappLogs3} className="text-xs text-muted-foreground hover:text-foreground transition flex items-center gap-1"><RotateCcw size={12} /> Atualizar</button>
                  </div>
                  {whatsappLogsLoading3 ? (
                    <div className="text-center py-8 text-muted-foreground text-sm animate-pulse">Carregando histórico...</div>
                  ) : whatsappLogs3.length === 0 ? (
                    <div className="text-center py-8 text-muted-foreground text-sm">Nenhuma mensagem enviada ainda.</div>
                  ) : (
                    <div className="max-h-[400px] overflow-y-auto space-y-2 pr-1 [&::-webkit-scrollbar]:w-1.5 [&::-webkit-scrollbar-track]:bg-transparent [&::-webkit-scrollbar-thumb]:bg-white/[0.1] [&::-webkit-scrollbar-thumb]:rounded-full">
                      {whatsappLogs3.map((log: any) => (
                        <div key={log.id} className="flex items-start gap-3 p-3 rounded-xl border border-white/[0.06] bg-white/[0.02] hover:bg-white/[0.04] transition">
                          <div className={`mt-0.5 w-2 h-2 rounded-full flex-shrink-0 ${log.status === 'sent' ? 'bg-green-400' : 'bg-red-400'}`} />
                          <div className="flex-1 min-w-0">
                            <div className="flex items-center gap-2 mb-1">
              ...
              <GlassCard className="p-5 space-y-3">
                ...
                {/* Poll editor (only relevant when sending to groups) */}
                {(notifySelectedGroups.length > 0 || notifySelectedGroups3.length > 0) && (
                  <div className="space-y-2 border border-white/[0.08] rounded-xl p-3 bg-white/[0.02]">
                    <label className="flex items-center gap-2 text-xs cursor-pointer select-none">
                      <input type="checkbox" checked={groupPoll.enabled} onChange={e => setGroupPoll(p => ({ ...p, enabled: e.target.checked }))} className="rounded border-white/20 bg-white/[0.04]" />
                      <BarChart3 size={14} className="text-blue-400" />
              ...
              <BulkSendProgress total={whatsappProgress3.total} sent={whatsappProgress3.sent} errors={whatsappProgress3.errors} skipped={whatsappProgress3.skipped} label="Disparo de WhatsApp 3" accent="green" />
              <BulkSendControls control={whatsapp3Ctrl.active ? whatsapp3Ctrl : whatsapp3GroupCtrl} visible={whatsappSending3} />

              <button
                onClick={async () => {
                  if (!evolutionApiUrl3 || !evolutionApiKey3 || !evolutionInstance3) { toast.error('Configure as credenciais da Evolution API'); setShowWhatsappConfig3(true); return; }
                  let waPhoneList: { phone: string; name: string }[] = [];
                  if (whatsappSourceMode === 'csv') {
                    waPhoneList = getSelectedExternalPhoneList();
                  } else {
                  ...
                  if (!whatsappMessage.trim() && !whatsappMedia) { toast.error('Digite a mensagem ou anexe uma mídia'); return; }
                  if (!await confirmDialog({ title: 'Confirmar disparo de WhatsApp', message: `Enviar esta mensagem para ${phones.length} contato(s)?`, variant: 'info', confirmLabel: 'Disparar' })) return;
                  setWhatsappSending3(true);
                  setWhatsappProgress3({ total: phones.length, sent: 0, errors: 0, skipped: 0 });
                  whatsapp3Ctrl.start();
                  let sent = 0, errors = 0;
                  for (let i = 0; i < phones.length; i++) {
                    if (await whatsapp3Ctrl.shouldStop()) break;
                    const phone = phones[i];
                    const matchedUser = waPhoneList.find(p => p.phone === phone);
                    try {
                      const { data: respData, error } = await supabase.functions.invoke('send-whatsapp3', { body: { recipientPhone: phone, message: whatsappMessage, evolutionApiUrl: evolutionApiUrl3, evolutionApiKey: evolutionApiKey3, evolutionInstance: evolutionInstance3, media: whatsappMedia || undefined, mentionsEveryOne: whatsappMentionAll || undefined } });
                      const hasError = !!error || !!respData?.error;
                      const errorMsg = error?.message || respData?.error || null;
                      await (supabase as any).from('whatsapp3_message_log').insert({
                        owner_id: session.user.id,
                        recipient_phone: phone,
                        recipient_name: matchedUser?.name || '',
                        message: whatsappMessage,
                      ...
                      if (hasError) { errors++; setWhatsappProgress3(p => ({ ...p, errors: p.errors + 1 })); }
                      else { sent++; setWhatsappProgress3(p => ({ ...p, sent: p.sent + 1 })); }
                    } catch (e: any) {
                      errors++;
                      setWhatsappProgress3(p => ({ ...p, errors: p.errors + 1 }));
                      await (supabase as any).from('whatsapp3_message_log').insert({
                        owner_id: session.user.id,
                        recipient_phone: phone,
                        recipient_name: matchedUser?.name || '',
                  ...
                  whatsapp3Ctrl.finish();
                  setWhatsappSending3(false);
                  if (errors > 0) toast.error(`${sent} enviado(s), ${errors} erro(s)`);
                  else toast.success(`${sent} mensagem(ns) enviada(s)!`);
                  setTimeout(() => setWhatsappProgress3(emptyProgress), 4000);
                }}
                disabled={whatsappSending3}
                className="w-full py-3.5 rounded-xl bg-green-600 hover:bg-green-500 text-white font-bold text-sm disabled:opacity-50 transition-all shadow-lg shadow-green-600/20 flex items-center justify-center gap-2"
              >
                {whatsappSending3 ? <><div className="w-4 h-4 border-2 border-white border-t-transparent rounded-full animate-spin" /> Enviando...</> : <><Send size={16} /> Enviar WhatsApp</>}
              </button>
              ...
              {notifySelectedGroups3.length > 0 && (
                <button
                  onClick={async () => {
                    ...
                    if (groupPoll.enabled && !pollPayload) return;
                    if (!pollPayload && !whatsappMessage.trim() && !whatsappMedia) { toast.error('Digite a mensagem ou anexe uma mídia'); return; }
                    if (!await confirmDialog({ title: pollPayload ? 'Confirmar enquete para Grupos' : 'Confirmar disparo para Grupos', message: pollPayload ? `Enviar enquete para ${notifySelectedGroups3.length} grupo(s)?` : `Enviar esta mensagem para ${notifySelectedGroups3.length} grupo(s)?`, variant: 'info', confirmLabel: 'Disparar' })) return;
                    setWhatsappSending3(true);
                    setWhatsappProgress3({ total: notifySelectedGroups3.length, sent: 0, errors: 0, skipped: 0 });
                    whatsapp3GroupCtrl.start();
                    let sent = 0, errors = 0;
                    for (const group of notifySelectedGroups3) {
                      if (await whatsapp3GroupCtrl.shouldStop()) break;
                      try {
                        const { data: respData, error } = await supabase.functions.invoke('send-whatsapp3', {
                          body: { recipientPhone: group.id, message: pollPayload ? '' : whatsappMessage, evolutionApiUrl: evolutionApiUrl3, evolutionApiKey: evolutionApiKey3, evolutionInstance: evolutionInstance3, media: pollPayload ? undefined : (whatsappMedia || undefined), mentionsEveryOne: whatsappMentionAll || undefined, poll: pollPayload || undefined }
                        });
                        const hasError = !!error || !!respData?.error;
                        if (hasError) { errors++; setWhatsappProgress3(p => ({ ...p, errors: p.errors + 1 })); }
                        else { sent++; setWhatsappProgress3(p => ({ ...p, sent: p.sent + 1 })); }
                  ...
                  className="w-full py-3.5 rounded-xl bg-blue-600 hover:bg-blue-500 text-white font-bold text-sm disabled:opacity-50 transition-all shadow-lg shadow-blue-600/20 flex items-center justify-center gap-2"
                >
                  {whatsappSending3 ? <><div className="w-4 h-4 border-2 border-white border-t-transparent rounded-full animate-spin" /> Enviando...</> : <><Users size={16} /> Enviar para {notifySelectedGroups3.length} Grupo(s)</>}
                </button>
               )}
                ...
                {showScheduler && (
                  <div className="space-y-4">
                    ...
                    <div className="space-y-2">
                      ...
                      {scheduledLoading ? (
                        <p className="text-xs text-muted-foreground text-center py-4">Carregando...</p>
                      ) : scheduledMessages.filter((m: any) => m.channel === 'whatsapp3').length === 0 ? (
                        <p className="text-xs text-muted-foreground text-center py-4">Nenhum agendamento</p>
                      ) : (
                        <div className="space-y-2 max-h-80 overflow-y-auto">
                          {scheduledMessages.filter((m: any) => m.channel === 'whatsapp3').map((m: any) => (
                            <div key={m.id} className={`p-3 rounded-xl border text-xs space-y-1 ${m.status === 'pending' ? 'border-primary/20 bg-primary/5' : m.status === 'sent' ? 'border-green-500/20 bg-green-500/5' : m.status === 'cancelled' ? 'border-muted/20 bg-muted/5 opacity-60' : 'border-red-500/20 bg-red-500/5'}`}>
                              <div className="flex justify-between items-start">
                                <span className="font-medium text-foreground truncate max-w-[70%]">{m.recipient_label || m.recipient_value}</span>
                                <span className={`px-2 py-0.5 rounded-full text-[10px] font-bold ${m.status === 'pending' ? 'bg-primary/20 text-primary' : m.status === 'sent' ? 'bg-green-500/20 text-green-400' : m.status === 'cancelled' ? 'bg-muted/20 text-muted-foreground' : 'bg-red-500/20 text-red-400'}`}>
          ...
          {activeTab === 'sorteio_whatsapp' && session?.user?.id && (
            <Suspense fallback={<PanelFallback />}>
              <WhatsappActivityPanel ownerId={session.user.id} />
            </Suspense>
          )}
          {activeTab === 'referral' && (
            <div className="max-w-2xl space-y-5">
