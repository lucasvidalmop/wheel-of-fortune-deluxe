...
const Registration = () => {
  ...
  useEffect(() => {
    ...
    if (seoConfig.googleAnalyticsId) {
      ...
      gtagInit.innerHTML = `window.dataLayer=window.dataLayer||[];function gtag(){dataLayer.push(arguments)}gtag('js',new Date());gtag('config','${seoConfig.googleAnalyticsId}');`;
      document.head.appendChild(gtagInit);
      cleanups.push(() => { gtagScript.remove(); gtagInit.remove(); });
    }
    ...
    if (seoConfig.gtmId) {
      ...
      cleanups.push(() => gtmScript.remove());
    }
    ...
    if (seoConfig.tiktokPixelId) {
      ...
      cleanups.push(() => ttScript.remove());
    }

    // Custom head script
    if (seoConfig.customHeadScript) {
      const customEl = document.createElement('div');
      customEl.innerHTML = seoConfig.customHeadScript;
      Array.from(customEl.children).forEach(child => {
